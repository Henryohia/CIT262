...
`).length===1}isMultilineStatement(e){return E(`<?php${e}?>`,this.options).trimRight().split(`
...
`;m.stdout.write(e),m.stdout.write($.bold.green(`Fixed: F
`)),m.stdout.write($.bold.red(`Errors: E
`)),m.stdout.write($.bold("Not Changed: ")+$.bold.green(`.
`))}printFormattedFiles(){if(this.formattedFiles.length===0){this.options.checkFormatted&&m.stdout.write($.bold(`
...
`);return}m.stdout.write($.bold(`
Formatted Files: 
`)),_.each(this.formattedFiles,e=>m.stdout.write(`${$.bold(e)}
`))}printDifferences(){if(this.options.diff){if(m.stdout.write($.bold(`
Differences: 

`)),_.filter(this.diffs,e=>e.length>0).length===0){m.stdout.write($(`No changes found. 

`));return}_.each(this.diffs,e=>de(e))}}printErrors(){_.isEmpty(this.errors)||(m.stdout.write($.red.bold(`
Errors: 

`)),_.each(this.errors,e=>m.stdout.write(`${Se.format(e)}
